...
Transaction.getAllByDatesByUser = (data) => {

    console.log("getAllByDatesByUser:",data);
    const userId = [...data];
    const QUERY = `
    with transactions as (
        select inc.income_id as transaction_id,inc.account_id,cat.category_name,inc.amount,inc.created_date,'income' as type,curr.description as currency,inc.created_time from income inc, account ac,category cat,currency curr where 1=1
        ...
        select t.transaction_id,t.account_id,t.category_name,t.amount,t.created_date,t.type,t.currency,to_char(t.created_time, 'HH24:MI') as created_time from transactions t where created_date>=$2 and created_date<=$3 order by created_date desc,created_time desc
    `;
    try {
        return pgdb.query(QUERY,userId);    
    } catch (error) {
